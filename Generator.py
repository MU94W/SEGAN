...
class G(object):
    ...
    def __call__(self, inp, scope=None, reuse=None):
        ...
        with tf.variable_scope(scope or self.name, reuse=reuse):
            ...
            assert feat_map == feature_map_lst[0], "[!] input's last dim must be 1."
            ...
            with tf.variable_scope("encode"):
                ...
                for i in range(1, len(feature_map_lst)):
                    with tf.variable_scope("stage_%d" % i):
                        ...
                        this_out = tf.contrib.keras.layers.PReLU()(tf.nn.conv1d(last_out, this_filter, stride, padding))
                        per_layer_out.append(this_out)
                        last_out = this_out

            ### decode stage
            with tf.variable_scope("decode"):
                latent_z = tf.expand_dims(tf.random_normal(shape=tf.shape(per_layer_out[-1])), axis=1)
                last_out = latent_z
                out_channel_lst = feature_map_lst[::-1][1:]
                batch_size = tf.shape(latent_z)[0]
                for out_channles, res_out, idx in zip(out_channel_lst, per_layer_out[::-1], range(1, len(feature_map_lst))):
                    with tf.variable_scope("stage_%d" % idx):
                        ...
                        this_out = tf.contrib.keras.layers.PReLU()(tf.reshape(tf.nn.conv2d_transpose(last_out_conc, this_filter, \
                                output_shape=output_shape, strides=[1, 1, stride, 1], padding=padding), shape=output_shape))
                        last_out = this_out
